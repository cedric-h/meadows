...
typedef struct {
  uint32_t id;
  Person person; /* probably needa couple to tween */
...
WASM_EXPORT void init(void) {
  ...
  person_frames_fill(&state.mf);
  typedef struct { PersonPartKind lhs, rhs; } PartPair;
  PartPair mirror[] = {
    { PersonPartKind_Knee_L , PersonPartKind_Knee_R  },
    { PersonPartKind_Sole_L , PersonPartKind_Sole_R  },
    { PersonPartKind_Toe_L  , PersonPartKind_Toe_R   },
    { PersonPartKind_Elbow_L, PersonPartKind_Elbow_R },
    { PersonPartKind_Hand_L , PersonPartKind_Hand_R  },
  };
  PersonPartKind no[] = {
    PersonPartKind_Elbow_L, PersonPartKind_Elbow_R,
    PersonPartKind_Hand_L , PersonPartKind_Hand_R ,
  ...
  /* pull arms in (less chonky stick person) */
  for (int j = 0; j < PersonPartKind_COUNT; j++)
    for (int i = 0; i < ARR_LEN(no); i++) {
      PersonPartKind n = no[i];
  ...
  for (int j = 0; j < 3; j++) {
    for (int i = 0; i < ARR_LEN(mirror); i++) {
      PersonPartKind lhs = mirror[i].lhs;
      PersonPartKind rhs = mirror[i].rhs;
  ...
  for (int j = 0; j < PersonPartKind_COUNT; j++) {
    for (int i = 0; i < ARR_LEN(no); i++) {
      PersonPartKind n = no[i];
      float center = state.mf.frames[j].pos[PersonPartKind_Neck].y;
...
static void person_anim(Person *person, float dt, Vec2 vel) {
  uint8_t going = dot2(vel, vel) > 0.00001f;
  person->anim_damp = lerp(person->anim_damp, going, dt * 0.15f);
  person->anim_prog += 0.14f*person->anim_damp*dt;
  int anim_len = ARR_LEN(state.mf.frames);
  person->anim_prog = lerp_round(anim_len,
    person->anim_prog, going*person->anim_prog, dt*0.05f);
  person->anim_prog = fmodf(person->anim_prog, anim_len);

  person->dir = lerp_rad(person->dir, atan2f(-vel.y, -vel.x), going*dt*0.1f);
}

static Vec2 person_pos(Person *person, PersonPartKind mpk) {
  float q = person->anim_prog;
  ...
  float mx = cosf(person->dir);
  float my = sinf(person->dir);
  Mat4 m = look_at4x4((Vec3) { mx, my, 0.75f },
                      (Vec3) {  0,  0, 0     },
                      (Vec3) {  0,  0, 1     });
  p = mul4x44(m, p);
  return (Vec2) { p.x * 0.5f + person->pos.x,
                  p.y * 0.5f + person->pos.y };
}

static void geo_person(Geo *geo, Person *person, Color skin_color) {
  float z = person_pos(person, PersonPartKind_Toe_R).y - 0.1f;

  Vec2 head = person_pos(person, PersonPartKind_Head);
  geo_ngon(geo, skin_color, z, head.x, head.y, 0.08f, 32);

  typedef struct { PersonPartKind lhs, rhs; } PartPair;
  PartPair pp[] = {
    { PersonPartKind_Neck,    PersonPartKind_Pelvis  },
    { PersonPartKind_Neck,    PersonPartKind_Elbow_R },
    { PersonPartKind_Neck,    PersonPartKind_Elbow_L },
    { PersonPartKind_Elbow_R, PersonPartKind_Hand_R  },
    { PersonPartKind_Elbow_L, PersonPartKind_Hand_L  },
    { PersonPartKind_Pelvis,  PersonPartKind_Knee_R  },
    { PersonPartKind_Pelvis,  PersonPartKind_Knee_L  },
    { PersonPartKind_Knee_R,  PersonPartKind_Sole_R  },
    { PersonPartKind_Knee_L,  PersonPartKind_Sole_L  },
    { PersonPartKind_Sole_R,  PersonPartKind_Toe_R   },
    { PersonPartKind_Sole_L,  PersonPartKind_Toe_L   },
  };

  float thickness = 0.0125f;
  for (PersonPartKind i = 0; i < PersonPartKind_COUNT; i++) {
    Vec2 pos = person_pos(person, i);
    geo_8gon(geo, skin_color, z, pos.x, pos.y, thickness);
  }

  for (int i = 0; i < ARR_LEN(pp); i++)
    geo_line(geo, skin_color, z,
      person_pos(person, pp[i].lhs),
      person_pos(person, pp[i].rhs),
      thickness*2.0f);
}

static void geo_person_id(Geo *geo, Person *person, uint32_t id) {
  Vec3 skin_color3 = lerp3(
    (Vec3) { 0.20f, 0.25f, 0.43f },
    (Vec3) { 0.26f, 0.19f, 0.43f },
    (float)id/(float)(UINT32_MAX));

  geo_person(geo, person, (Color) {
    .r = skin_color3.x,
    .g = skin_color3.y,
    .b = skin_color3.z,
    .a = 1.0f,
...
static void quest(Geo *geo, Mushroom **onscreen_mush, float dt) {
  ...
  uint8_t near_wiz = mag2(sub2(wiz, state.player.person.pos)) < 0.8f;
    ...
    case QuestStage_MushPickin: {
      ...
      for (int i = 0; onscreen_mush[i]; i++) {
        ...
        if (m->stage == MushroomStage_Ripe && 
            mag2(sub2(m->pos, state.player.person.pos)) < 0.4f)
        {
          SHOW_TODO(2, "- (press E)");
...
WASM_EXPORT void frame(int width, int height, double _dt) {
  ...
  if (state.time_since_netout > 0.2f) {
    ...
    NetMsg msg = { state.id, state.player.person };
  ...
  switch (state.player.action) {
    case PlayerAction_Walking: {
      ...
      if (dot2(state.player.vel, state.player.vel) > 0.00008f) {
        ...
        state.player.person.pos = add2(state.player.person.pos,
                                    mul2_f(norm2(state.player.vel),
                                           0.013f*state.player.person.anim_damp));
      }

      person_anim(&state.player.person, dt, state.player.vel);
    ...
    case PlayerAction_Casting: {
      ...
      person_anim(&state.player.person, dt, (Vec2){});

      Color magic = { 0.2f, 0.64f, 0.8f, 1.0f };
      Vec2 p = add2(state.player.cast_target, state.player.person.pos);
      for (int u = 0; u < 5; u++) {
      ...
      if (!state.keys_down[' ']) {
        ...
        Vec2 target = p;
        Vec2 start = add2(state.player.person.pos, (Vec2) { 0, 0.75f });
  ...
  geo_person_id(&geo, &state.player.person, state.id);

  {
    static Person pacing_person = {0};
    static float t = 0.0f;
    t += dt;
    ...
    pacing_person.pos = pos;
    person_anim(&pacing_person, dt, vel);

    geo_person_id(&geo, &pacing_person, 0);
  }

  for (int i = 0; i < ARR_LEN(state.others); i++) {
    Other *other = state.others + i;
    if (other->id)
      geo_person_id(&geo, &other->person, other->id);
  }
  ...
  /* lerp cam towards player */
    ...
    Vec2 head = person_pos(&state.player.person, PersonPartKind_Head);
    Vec2 p = mul2_f(add2(state.player.person.pos, head), 0.5f);
    state.cam = (Vec2) { lerp(state.cam.x, -p.x + halsc.x, dt*0.04f),
                         lerp(state.cam.y, -p.y + halsc.y, dt*0.04f) };
  }

  // geo_person(&geo, dt, 0.3f*cosf(dt*0.001f+M_PI/2),
