...
#define POS(o) mul2_f(vpos[ibuf[o]], scale)
  for (int j = 0; j < ibuf_len; j += 3) {
    geo_tri(geo, (Vert) { .pos = add2(pos, POS(j+0)), .z = pos.y + z, .color = c },
...
void geo_pot(Geo *geo, float x, float y) {
  ...
  geo_geo(geo, pot_body_vpos, pot_body_idx, ARR_LEN(pot_body_idx),
               -0.0027f, pos, (Color) { 0.31f, 0.31f, 0.31f+0.1f, 1.00f }, scale);

  geo_geo(geo, pot_legs_vpos, pot_legs_idx, ARR_LEN(pot_legs_idx),
               -0.0024f, pos, (Color) { 0.27f, 0.27f, 0.27f+0.1f, 1.00f }, scale);

  geo_geo(geo, pot_lower_lip_vpos, pot_lower_lip_idx, ARR_LEN(pot_lower_lip_idx),
               -0.0029f, pos, (Color) { 0.28f, 0.28f, 0.28f+0.1f, 1.00f }, scale);

  geo_geo(geo, pot_top_vpos, pot_top_idx, ARR_LEN(pot_top_idx),
               -0.0028f, pos, (Color) { 0.34f, 0.34f, 0.34f+0.1f, 1.00f }, scale);
}
